...
public class MapHolder<K extends Identifiable, V> implements Map<Integer, V> {
    ...
    @Override public V put(Integer key, V value) {
        // УБРАНО: Жесткая проверка keyHolder.contains(key), которая приводила к падению,
        // когда MapHolder был создан через пустой конструктор и использовался со стандартными методами Java
